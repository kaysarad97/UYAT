  ...
  return (
    <>
      {incidents.length === 0 ? (
        ...
        <IncidentView incidents={incidents} setIncidents={setIncidents} /> // Передаем функцию setIncidents в IncidentView
      )}
      ...
      {incidentHistory.length === 0 ? (
        ...
        <IncidentView incidents={incidentHistory} /> // Историю инцидентов можно отобразить без setIncidents, если обновление не требуется
      )}
    </>
  );
}
